...
export default function SchedulePage({ searchQuery = '', isSignedIn = false, onNavigate }: SchedulePageProps) {
    ...
    const displaySessions = sessions.map(s => ({
        ...
        levelColor: s.level === 'Beginner' ? '#059669' : s.level === 'Intermediate' ? '#D97706' : s.level === 'Advanced' ? '#B91C1C' : '#0369A1',
    ...
    const filteredSessions = viewMode === 'my'
        ? searchFiltered.filter(s => s.isMySession)
        : searchFiltered;
    ...
    const handleAddToCalendar = (session: typeof displaySessions[0]) => {
        // Parse session.date (YYYY-MM-DD) and session.time (e.g. "9:00 AM – 10:00 AM") into ICS format
        const dateStr = session.date.replace(/-/g, ''); // YYYYMMDD
        const timeParts = session.time.split(/\s*[–-]\s*/); // split "9:00 AM – 10:00 AM"
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm mt-0.5" style={{ color: 'var(--text-secondary)' }}>Discover and join live tutoring sessions</p>
                </div>
                <button

...
export default function Dashboard({ onNavigate }: DashboardProps) {
  ...
  const courses = [
    { name: 'Algebra Fundamentals', progress: 72, total: 24, completed: 17, color: 'var(--brand)' },
    { name: 'Biology 101', progress: 45, total: 20, completed: 9, color: 'var(--success)' },
    { name: 'English Literature', progress: 88, total: 16, completed: 14, color: 'var(--warning)' },
    { name: 'World History', progress: 31, total: 30, completed: 9, color: 'var(--info)' },
    { name: 'Chemistry Basics', progress: 60, total: 18, completed: 11, color: 'var(--purple)' },
  ];

  const weekDays = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
  const weekActivity = [45, 30, 60, 25, 55, 40, 0]; // minutes per day
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl p-5 lg:p-6 border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button
            ...
            className="flex items-center gap-3 p-3 rounded-xl transition-colors text-left min-h-[48px] border"
            style={{ backgroundColor: 'var(--brand-bg)', borderColor: 'var(--border-light)' }}
          >
            <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--brand-bg)' }}>
              <Play size={18} style={{ color: 'var(--brand)' }} />
            </div>
            <div>
              <div className="text-xs" style={{ color: 'var(--text-secondary)' }}>Continue</div>
              <div className="text-sm font-semibold" style={{ color: 'var(--text)' }}>Algebra Ch. 7</div>
            </div>
          </button>
          <button
            onClick={() => onNavigate('schedule')}
            className="flex items-center gap-3 p-3 rounded-xl transition-colors text-left min-h-[48px] border"
            style={{ backgroundColor: 'var(--success-bg)', borderColor: 'var(--border-light)' }}
          >
          ...
          <button
            onClick={() => onNavigate('resources')}
            className="flex items-center gap-3 p-3 rounded-xl transition-colors text-left min-h-[48px] border"
            style={{ backgroundColor: 'var(--warning-bg)', borderColor: 'var(--border-light)' }}
          >
            <div className="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--warning-bg)' }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="rounded-xl p-5 lg:p-6 border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <button
            ...
            className="flex items-center gap-1 mt-4 text-sm font-medium min-h-[44px]"
        ...
        <div className="rounded-xl p-5 lg:p-6 border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs font-medium px-2 py-1 rounded-full flex items-center gap-1" style={{ backgroundColor: 'var(--success-bg)', color: 'var(--success)' }}>
              ...
              <div key={day} className="flex-1 flex flex-col items-center gap-1">
                ...
                <div className="w-full rounded-t-md relative" style={{ height: '100%', backgroundColor: 'var(--bg)' }}>
                  <div
                    ...
                    style={{
                      height: `${maxActivity > 0 ? (weekActivity[i] / maxActivity) * 100 : 0}%`,
          ...
          <div className="flex items-center gap-4 pt-3 border-t text-sm" style={{ borderColor: 'var(--border)' }}>
            <div>
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>Total this week</span>
              <div className="font-bold" style={{ color: 'var(--text)' }}>{weekActivity.reduce((a, b) => a + b, 0)} min</div>
            </div>
            <div>
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>Avg per day</span>
              <div className="font-bold" style={{ color: 'var(--text)' }}>{Math.round(weekActivity.reduce((a, b) => a + b, 0) / 7)} min</div>
            </div>
            <div>
              <span className="text-xs" style={{ color: 'var(--text-secondary)' }}>Items done</span>
              <div className="font-bold" style={{ color: 'var(--text)' }}>14</div>
            </div>
          </div>

          <button
            onClick={() => setShowMoreActivity(!showMoreActivity)}
            className="flex items-center gap-1 mt-3 text-sm font-medium min-h-[44px]"
            style={{ color: 'var(--brand)' }}
          >
          ...
          {showMoreActivity && (
            <div className="mt-2 p-3 rounded-lg text-sm" style={{ backgroundColor: 'var(--bg)', color: 'var(--text-secondary)' }}>
              <p>📊 You're studying more consistently than last month — up 15% in daily minutes.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="rounded-xl p-5 lg:p-6 border" style={{ backgroundColor: 'var(--card)', borderColor: 'var(--border)' }}>
          ...
          <p className="text-xs text-center p-2 rounded-lg" style={{ backgroundColor: 'var(--success-bg)', color: 'var(--text-secondary)' }}>
            <CheckCircle2 size={12} className="inline mr-1" style={{ color: 'var(--success)' }} />
            Complete today's task to keep your streak!
          </p>
        </div>
            ...
            {[
              {
                ...
                desc: 'Next in Algebra — builds on Ch. 7',
                type: 'Lesson',
                time: '20 min',
                icon: BookOpen,
                color: 'var(--brand)',
                colorBg: 'var(--brand-bg)',
              },
              {
                title: 'Cell Division Quiz',
                desc: 'Test your Bio knowledge',
                type: 'Quiz',
                time: '10 min',
                icon: Target,
                color: 'var(--success)',
              ...
              {
                title: 'Study Group: Algebra',
                desc: '5 students online now',
                type: 'Live',
                time: 'Now',
                icon: Star,
                color: 'var(--warning)',
              ...
              <button
                key={i}
                onClick={() => onNavigate('resources')}
                className="p-4 rounded-xl border transition-all hover:shadow-md text-left min-h-[48px]"
                style={{ borderColor: 'var(--border)' }}
              >
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ backgroundColor: rec.colorBg }}>
                    <rec.icon size={16} style={{ color: rec.color }} />
                  </div>
                  <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: rec.colorBg, color: rec.color }}>
                    {rec.type}
                  </span>
                </div>
